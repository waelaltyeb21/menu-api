...
// Get Order
const GetOrderForClient = async (req, res) => {
  const { id } = req.params;
  try {
    if (isValidObjectId(id)) {
      ...
      const [order] = await OrderModel.aggregate([
        {
          $match: {
            table: new mongoose.Types.ObjectId(id),
          },
        },
        {
          $unwind: "$orders",
      ...
      const statusStages = {
        isRequested: "في انتظار الموافقة على الطلب",
        isProccessing: "تم قبول طلبك",
        isPaid: "تم الدفع واكمال الطلب بنجاح",
      };
      const status = statusStages[order.orderStatus];
      const orderDetails = { ...order, orderStatus: status };

      if (order) return res.status(200).json(orderDetails);
...
const CreateNewOrder = async (req, res) => {
  const { orders, restaurant, table, total, user, orderStatus, isClient } =
    req.body;
  try {
    if (isValidObjectId(table) && isValidObjectId(restaurant)) {
      ...
      // If Client Allready Order
      console.log("Check Client ? ", isClient);
      if (isClient) {
        UpdateOrder(req, res);
      }
...
const ComplateOrder = async (req, res) => {
...
const getOrdersOfWeek = async (req, res) => {
